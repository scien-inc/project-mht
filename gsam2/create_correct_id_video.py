...
if __name__ == "__main__":
    import argparse  # argparseのインポート

    # コマンドライン引数をパース
    parser = argparse.ArgumentParser(description='Video Processor Script')
    parser.add_argument('--input_folder', type=str, required=True, help='入力フレーム画像が保存されているフォルダのパス')
    parser.add_argument('--mask_data_dir', type=str, required=True, help='マスクが保存されているディレクトリのパス')
    parser.add_argument('--json_data_dir', type=str, required=True, help='JSONデータが保存されているディレクトリのパス')
    parser.add_argument('--result_dir', type=str, required=True, help='結果を保存するディレクトリのパス')
    parser.add_argument('--output_video_path', type=str, required=True, help='出力ビデオのパス')
    parser.add_argument('--frame_rate', type=int, default=5, help='ビデオのフレームレート')
    ...
    # 結果の描画とビデオの保存を実行
    CommonUtils.draw_masks_and_box_with_supervision(
        args.input_folder, args.mask_data_dir, args.json_data_dir, args.result_dir
    )
    create_video_from_images(args.result_dir, args.output_video_path, frame_rate=args.frame_rate)
